(()=>{
  ...
  function navLocator(){
    ...
    const links=()=>$$('a',nav);
    const move=a=>{if(!a||innerWidth<760)return;cancelAnimationFrame(navFrame);navFrame=requestAnimationFrame(()=>{const x=a.offsetLeft-6,w=a.offsetWidth+12;loc.style.width=`${w}px`;loc.style.transform=`translate3d(${x}px,-50%,0)`;nav.classList.add('ei-ready')})};
    const home=()=>move(links().find(a=>a.classList.contains('active'))||links()[0]);
    links().forEach(a=>{if(a.dataset.eiNavBound==='1')return;a.dataset.eiNavBound='1';a.addEventListener('pointerenter',()=>move(a),{passive:true});a.addEventListener('focus',()=>move(a));a.addEventListener('pointerdown',()=>move(a),{passive:true});a.addEventListener('click',()=>routeFlash())});
    nav.onpointerleave=home;nav.addEventListener('focusout',e=>{if(!nav.contains(e.relatedTarget))home()});home();
    if(!nav.dataset.eiResizeBound){nav.dataset.eiResizeBound='1';addEventListener('resize',home,{passive:true})}
  }

  function routeFlash(){if(reduced)return;let f=$('.ei-route-flash');if(!f){f=document.createElement('i');f.className='ei-route-flash';document.body.appendChild(f)}const y=Math.round($('.site-nav')?.getBoundingClientRect().bottom||0);f.style.setProperty('--ei-route-y',`${y}px`);f.classList.remove('run');void f.offsetWidth;f.classList.add('run')}
  ...
  function scanPanels(){if(reduced||!('IntersectionObserver'in window))return;if(!scanObserver)scanObserver=new IntersectionObserver(entries=>entries.forEach(e=>{if(!e.isIntersecting)return;const el=e.target;if(el.dataset.eiScanned==='1'){scanObserver.unobserve(el);return}el.dataset.eiScanned='1';el.classList.add('ei-scan-live');setTimeout(()=>el.classList.remove('ei-scan-live'),760);scanObserver.unobserve(el)}),{threshold:.3,rootMargin:'0px 0px -8%'});[...new Set(scanSelectors.flatMap(s=>$$(s)))].forEach(el=>{if(el.dataset.eiScanBound==='1'||el.closest('[hidden]'))return;el.dataset.eiScanBound='1';el.classList.add('ei-kinetic-scan');if(!$(':scope > .ei-scan-line',el)){const line=document.createElement('i');line.className='ei-scan-line';el.appendChild(line)}scanObserver.observe(el)})}
  ...
  function schedule(){clearTimeout(enhanceTimer);enhanceTimer=setTimeout(enhance,120)}
