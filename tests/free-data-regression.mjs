...
ok(/DecodeVinValuesExtended/.test(vin),'deep NHTSA VIN decode remains active');ok(/transmissionStyle/.test(vin)&&/electrificationLevel/.test(vin)&&/basePrice/.test(vin),'extended VIN attributes remain wired');ok(/free-evidence-v15\.js/.test(check),'free evidence UI loaded on Check');ok(/free-evidence-v15\.css/.test(check),'free evidence styles loaded on Check');ok(/api\/free-evidence/.test(client),'client uses free evidence endpoint');ok(/fetchHousePriceContext/.test(api)&&/fetchMortgageRates/.test(api),'free FHFA/Freddie context remains wired');ok(/Entered \/ pasted/.test(client),'verified pasted terms remain visibly labeled');ok(/never a property appraisal or transaction comp/i.test(client),'housing context cannot masquerade as property comp');
