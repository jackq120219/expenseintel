...
module.exports=async function handler(req,res){
  ...
  let body=req.body;if(typeof body==='string'){try{body=JSON.parse(body)}catch(_e){body={}}}body=body||{};
  ...
  try{const out=await gather({text,url,category,price,location});return send(res,200,out)}catch(e){return send(res,502,{ok:false,error:String(e?.message||e||'ExpenseIntel Check could not complete.')})}
};