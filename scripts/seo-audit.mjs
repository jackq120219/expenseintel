...
for(const p of pages){
  ...
  const checks=[['title',/<title>[^<]{8,}<\/title>/i],['description',/<meta[^>]+name=["']description["'][^>]+content=["'][^"']{40,}["']/i],['canonical',/<link[^>]+rel=["']canonical["'][^>]+href=["']https:\/\//i],['robots',/<meta[^>]+name=["']robots["']/i]];
...
if(bad)process.exit(1);console.log(`SEO AUDIT OK: ${pages.length} key pages + sitemap/robots`);
