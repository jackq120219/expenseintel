(()=>{
  ...
  function railSync(){const rail=makeRail(),nav=$('.site-nav'),max=Math.max(1,document.documentElement.scrollHeight-innerHeight),p=Math.max(0,Math.min(1,scrollY/max));$('i',rail).style.transform=`scaleX(${p})`;rail.style.top=`${Math.max(0,Math.round(nav?.getBoundingClientRect().bottom||0))}px`;const sections=$$('main section').filter(s=>s.offsetParent!==null);let active=sections[0];const probe=innerHeight*.34;for(const s of sections){if(s.getBoundingClientRect().top<=probe)active=s;else break}const label=active?sectionLabel(active):'ExpenseIntel';if(label&&label!==currentLabel){currentLabel=label;$('span',rail).textContent=label;rail.classList.add('show-label');clearTimeout(labelTimer);labelTimer=setTimeout(()=>rail.classList.remove('show-label'),1100)}}
  function installRail(){makeRail();railSync();let ticking=false;const tick=()=>{if(ticking)return;ticking=true;requestAnimationFrame(()=>{railSync();ticking=false})};addEventListener('scroll',tick,{passive:true});addEventListener('resize',tick);if('ResizeObserver'in window)new ResizeObserver(tick).observe(document.body)}
  ...
  function init(){loadSystems();installRail();installScanWatch();installValueTraces()}
