(()=>{
  ...
  function model(v){
    ...
    running+=v.shock;yearly.push({label:'Shock',value:running});running=Math.max(0,running-v.recovery);yearly.push({label:'Exit',value:running});
    return{entry,carry,total:running,deltaTotal:running-v.base,points:yearly};
  }
  function chart(v,m){
    ...
    const labels=$('#tw-labels');labels.innerHTML=pts.map(q=>q.label==='Entry'||q.label==='Exit'||/^Yr [13579]$/.test(q.label)?`<text class="axis-label" x="${q.x.toFixed(1)}" y="${H-16}" text-anchor="middle">${q.label}</text>`:'').join('');
  ...
  function update(changed='entry'){
    lastChanged=changed;const v=values(),m=model(v);$('#tw-delta-out').textContent=`${v.delta>=0?'+':'−'}${money(Math.abs(v.delta))}`;$('#tw-growth-out').textContent=`${v.growth.toFixed(1).replace('.0','')}%`;$('#tw-years-out').textContent=`${v.years} yr${v.years===1?'':'s'}`;chart(v,m);impact(v,m);write(TWIN,{base:v.base,delta:v.delta,annual:v.annual,growth:v.growth,years:v.years,shock:v.shock,recovery:v.recovery,title:scenarioTitle(),updatedAt:new Date().toISOString()});
  }
  function bind(){
    const map=[['tw-base','entry'],['tw-delta','entry'],['tw-annual','carry'],['tw-growth','carry'],['tw-years','carry'],['tw-shock','shock'],['tw-recovery','exit']];map.forEach(([id,type])=>$('#'+id)?.addEventListener('input',()=>update(type)));$('#tw-title')?.addEventListener('input',()=>update(lastChanged));
    $('#tw-reset')?.addEventListener('click',()=>{if(active.price)$('#tw-base').value=active.price;else $('#tw-base').value='';$('#tw-delta').value=0;$('#tw-annual').value='';$('#tw-growth').value=0;$('#tw-years').value=5;$('#tw-shock').value='';$('#tw-recovery').value='';update('entry')});
    $('#tw-copy')?.addEventListener('click',async e=>{const v=values(),m=model(v),text=`ExpenseIntel Decision Twin — ${scenarioTitle()}\nKnown amount: ${money(v.base)}\nScenario entry: ${money(m.entry)}\nRecurring burden: ${money(m.carry)} over ${v.years} years\nOne-time downside: ${money(v.shock)}\nRecovery / exit: ${money(v.recovery)}\nScenario cash commitment: ${money(m.total)}\nDifference vs known amount: ${m.deltaTotal>=0?'+':'-'}${money(Math.abs(m.deltaTotal))}\nScenario only — assumptions are user-controlled.`;try{await navigator.clipboard.writeText(text);const old=e.currentTarget.textContent;e.currentTarget.textContent='Copied ✓';setTimeout(()=>e.currentTarget.textContent=old,1400)}catch(_e){}});
  ...
  function prefill(){
    $('#tw-title').value=activeTitle()==='No active Check yet'?(saved.title||''):activeTitle();$('#tw-base').value=Number(active.price)||Number(saved.base)||'';$('#tw-delta').value=Number(saved.delta)||0;$('#tw-annual').value=Number(saved.annual)||'';$('#tw-growth').value=Number(saved.growth)||0;$('#tw-years').value=Number(saved.years)||5;$('#tw-shock').value=Number(saved.shock)||'';$('#tw-recovery').value=Number(saved.recovery)||'';
    const base=Number($('#tw-base').value)||0,span=Math.max(1000,Math.min(base?base*0.25:50000,50000));$('#tw-delta').min=String(-Math.round(span/100)*100);$('#tw-delta').max=String(Math.round(span/100)*100);
