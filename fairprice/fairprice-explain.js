(()=>{
  ...
  function hydrateFromQuery(){const qs=new URLSearchParams(location.search),q=qs.get('q')||'',amount=Number(qs.get('amount'));const label=$('#fp-label'),quote=$('#fp-quote');if(q&&label&&!label.value)label.value=q;let parsed=Number.isFinite(amount)&&amount>0?amount:0;if(!parsed&&q){const a=q.match(/\$\s*([\d,.]+)/),b=q.match(/([\d,.]+)\s*\$/);parsed=Number(String((a?.[1]||b?.[1]||'')).replace(/,/g,''))||0}if(parsed&&quote&&(!quote.value||Number(quote.value)===17800)){quote.value=String(parsed);quote.dataset.autoFilled='1'}}
  ...
  document.addEventListener('DOMContentLoaded',()=>{restoreNav();hydrateFromQuery();const form=$('[data-fairprice-form]');if(!form)return;form.addEventListener('submit',()=>setTimeout(render,0));['fp-label','fp-quote','fp-scope','fp-low','fp-high','fp-comp1','fp-comp2','fp-comp3'].forEach(id=>$(`#${id}`)?.addEventListener('input',()=>{clearTimeout(window.__fpExplainTimer);window.__fpExplainTimer=setTimeout(render,180)}));setTimeout(render,80)});
})();