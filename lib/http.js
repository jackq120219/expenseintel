...
const DEFAULT_INTERNAL_ORIGIN='https://expenseintel.com';
...
function originFromReq(_req){
  const configured=process.env.EXPENSEINTEL_INTERNAL_ORIGIN||DEFAULT_INTERNAL_ORIGIN;
  try{
    const url=new URL(configured);
    if(url.protocol!=='https:')throw new Error('Internal origin must use HTTPS');
    return url.origin;
  }catch(error){
    console.error('Invalid EXPENSEINTEL_INTERNAL_ORIGIN; using production origin',error?.message||error);
