...
async function resolveFuelVehicle({year,make,model,limit=8}={}){
  ...
  try{
    ...
    let modelMatches=pickMatches(models,model,5);
    if(!modelMatches.length){const n=norm(model);modelMatches=models.filter(x=>norm(x.text).includes(n.slice(0,Math.max(3,n.length-1)))).slice(0,5)}
    ...
    const opts=[...optionMap.values()].slice(0,Math.max(1,Math.min(16,limit)));
