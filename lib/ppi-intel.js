...
const CATEGORY_KEYS={
  home:['construction','lumber','softwood_lumber','roofing','hvac','plumbing','paint','concrete'],
  property:['construction','property_insurance','residential_property_mgmt','hvac','lumber'],
...
function parseRow(row){const c=cells(row);if(c.length<5)return null;const label=c[0],numbers=c.slice(3).map(num).filter(v=>v!=null);if(numbers.length<2)return null;return{label:strip(label),groupCode:c[1]||'',itemCode:c[2]||'',yoy:numbers[0],latestMonth:numbers[numbers.length-1]}}
