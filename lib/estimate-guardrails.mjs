...
function optionalNumber(v){if(v==null||v==='')return null;const n=Number(v);return Number.isFinite(n)?n:null}
export function sanityCheck({category='other',price=null,commitment=null,savings=null,recovery=null,annualInsurance=null,annualMaintenance=null,monthlyPayment=null,term=null,due=null}={}){
  const issues=[];const p=optionalNumber(price),c=optionalNumber(commitment),s=optionalNumber(savings),r=optionalNumber(recovery),ins=optionalNumber(annualInsurance),maint=optionalNumber(annualMaintenance),pay=optionalNumber(monthlyPayment),t=optionalNumber(term),d=optionalNumber(due);
  if(c!=null&&c<0)issues.push('negative commitment');
  if(s!=null&&s<0)issues.push('negative savings');
  if(c!=null&&s!=null&&c>0&&s>c*.75)issues.push('savings exceeds 75% of commitment');
  if(r!=null&&p!=null&&p>0&&r>p*1.5)issues.push('recovery exceeds 150% of price without an appreciation case');
  if(category==='vehicle'&&p!=null&&p>0){if(ins!=null&&ins>p*.20)issues.push('annual insurance exceeds 20% of vehicle price');if(maint!=null&&maint>p*.20)issues.push('annual maintenance exceeds 20% of vehicle price');if(d!=null&&d>p*.60)issues.push('upfront cash exceeds 60% of vehicle price');}
  if(pay!=null&&pay<0)issues.push('negative payment');
  if(t!=null&&(t<1||t>180))issues.push('term outside plausible analysis bounds');
  return issues;
}
