...
const MODEL_VERSION='EI-LCE-2026.09.3';
...
function quality(grade){return({A:1,B:.8,C:.5,D:.15})[String(grade||'D').toUpperCase()]||.15}
function evidenceGrade({location,energy,risk}){
  const layers=[
    {name:'Address',weight:20,grade:location?.verified?'A':'D',status:location?.verified?'verified':'unverified'},
    {name:'Electricity price',weight:15,grade:energy?.electricity?.grade||'C',status:energy?.electricity?'observed':'modeled'},
    {name:'Natural gas price',weight:10,grade:energy?.gas?.grade||'C',status:energy?.gas?'observed':'modeled'},
    {name:'Usage intensity',weight:15,grade:energy?.intensity?.grade||'C',status:energy?.intensity?'benchmark':'modeled'},
    {name:'Hazard context',weight:10,grade:risk?.available?'A':'D',status:risk?.available?'observed':'unavailable'},
    {name:'Property tax',weight:10,grade:'C',status:'modeled'},
    {name:'Insurance',weight:8,grade:'C',status:'modeled'},
    {name:'Water / sewer',weight:6,grade:'C',status:'modeled'},
    {name:'Waste / other',weight:6,grade:'C',status:'modeled'}
  ];
  const score=Math.round(layers.reduce((sum,l)=>sum+l.weight*quality(l.grade),0));
  const grade=score>=90?'A':score>=75?'B':score>=60?'C':'D';
  const authoritative=layers.filter(l=>l.status==='verified'||l.status==='observed').length;
  return{grade,score,authoritative,total:layers.length,layers};
}
...
function estimateCase({address,use,sqft,location=null,energy=null,risk=null}){
  ...
  const uncertainty=location?.verified?clamp(.18-(evidence.score/100)*.085,.09,.16):.20;
  ...
  const evidenceLedger=[
    ...
    {layer:'Property tax',status:'modeled',source:'ExpenseIntel location-cost assumption',grade:'C'},
    {layer:'Insurance',status:'modeled',source:'ExpenseIntel location-cost assumption',grade:'C'},
    {layer:'Water / sewer',status:'modeled',source:'ExpenseIntel location-cost assumption',grade:'C'},
    {layer:'Waste / other',status:'modeled',source:'ExpenseIntel location-cost assumption',grade:'C'}
  ];
