(()=>{
  ...
  function parseRate(s){const m=String(s||'').replace(/,/g,'').match(/-?[\d.]+/);const n=m?Number(m[0]):null;return Number.isFinite(n)?n:null}
  ...
  function shell(){
    ...
    p.querySelector('[data-passport-save]')?.addEventListener('click',saveSnapshot);
    return p;
  }
  function sessionLocation(){try{return JSON.parse(sessionStorage.getItem('ei_last')||'null')?.location||null}catch(_e){return null}}
  function snapshot(){
    ...
    const total=txt('[data-full-total]');
    ...
    const hazardRisk=txt('[data-nri-risk]')||'';
    const id='EI-LOC-'+hash(`${address}|${use}|${sqft}`);
    return {address,use,sqft,range,total,risk,ev,driver,hazard,hazardRisk,id,location:sessionLocation(),electricRate:parseRate(txt('[data-eia-electric-rate]')),gasRate:parseRate(txt('[data-eia-gas-rate]'))};
  }
  function update(){
  ...
  function saveSnapshot(){
    ...
    let arr=[];try{arr=JSON.parse(localStorage.getItem(STORAGE)||'[]')}catch(_e){}
    const item={id:d.id,address:d.address,use:d.use,sqft:Number(d.sqft)||0,range:d.range,total:d.total,risk:d.risk,hazard:d.hazard,hazardRisk:d.hazardRisk,electricRate:d.electricRate,gasRate:d.gasRate,location:d.location,savedAt:new Date().toISOString()};
    arr=arr.filter(x=>x.id!==d.id);arr.unshift(item);arr=arr.slice(0,50);try{localStorage.setItem(STORAGE,JSON.stringify(arr))}catch(_e){}
    const b=$('[data-passport-save]');if(b){b.textContent='Saved to Watchlist ✓';setTimeout(()=>b.textContent='Save to Cost Watchlist',1800)}
  }
